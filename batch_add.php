...
$result = $mysqli->query("SELECT batch_code FROM batches WHERE batch_code LIKE 'SH%' ORDER BY batch_id DESC LIMIT 1");

if ($row = $result->fetch_assoc()) {
    $latestCode = $row['batch_code'];
    $number = (int) substr($latestCode, 2);
    $newCode = 'SH' . str_pad($number + 1, 4, '0', STR_PAD_LEFT);
} else {
    $newCode = 'SH1001';
}

$products = $mysqli->query("SELECT product_id, title FROM products ORDER BY title");
...
<div class="container-fluid page-wrap">
    ...
    <div class="page-title-box">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center">
            <div>
                <h4>Add New Manufacturing Batch</h4>
                <small class="text-muted">Create batch and manage raw material usage</small>
            </div>
    ...
    <form action="batch_save.php" method="POST" id="batchForm">
            ...
            <div class="sa-card-body">
                <div class="row g-3">
                    ...
                    <div class="col-6 col-md-4 col-lg-2">
                        ...
                        <input type="text" name="batch_code" id="batch_code" class="form-control" value="<?= $newCode; ?>" required>
                    </div>

                    <div class="col-6 col-md-4 col-lg-2">
                        <label class="form-label">Quantity Produced <span class="text-danger">*</span></label>
                        <input type="number" step="0.01" min="0" name="product_qty" id="product_qty" class="form-control" required>
                    </div>

                    <div class="col-12 col-md-4 col-lg-2">
                        <label class="form-label">Production Date</label>
                        <input type="date" name="production_date" id="production_date" class="form-control" value="<?= $today; ?>">
                    </div>
        ...
        <div class="sa-card" id="materialCard" style="display:none;">
            ...
            <div class="sa-card-body">
                        ...
                        <table class="table table-bordered align-middle" id="raw-material-table">
                            ...
                            <tbody>
                                <tr>
                                    <td>
                                        <select name="material_id[]" class="form-select material-select" required>
                                            ...
                                            ?>
                                                <option 
                                                    ...
                                                    data-name="<?= htmlspecialchars(strtoupper($row['material_name'])) ?>"
                                                    data-owner="<?= htmlspecialchars(strtoupper($row['owner_type'])) ?>"
                                                    data-unit="<?= htmlspecialchars($row['unit']) ?>"
                                                    data-stock="<?= htmlspecialchars($row['current_stock']) ?>"
                                                    data-rate="<?= htmlspecialchars($row['average_price']) ?>"
                                                >
                                        ...
                                        <input type="hidden" name="material_owner_company[]" class="material-owner-hidden">
                                    </td>

                                    <td><input type="text" class="form-control material-owner" readonly></td>
                                    <td><input type="text" name="unit[]" class="form-control material-unit" readonly></td>
                                    <td><input type="number" step="0.01" class="form-control current-stock" readonly></td>
                                    <td>
                                        <input type="number" name="quantity[]" class="form-control qty-used" step="0.001" min="0" required>
                                        <div class="auto-formula-note">Auto suggested: KHUJLI = BLK Qty × 0.165</div>
                                    </td>
                                    <td><input type="number" name="rate[]" class="form-control rate" step="0.01" min="0" readonly></td>
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <th>
                                        <input type="number" name="total_material_cost" id="total_material_cost" class="form-control" readonly>
                                    </th>
                                    <th></th>
                                </tr>
                            </tfoot>
                ...
                <div class="total-box mt-3">
                    ...
                    <input type="number" id="mobile_total_material_cost" class="form-control" readonly>
                </div>

                <div class="text-end mt-3 desktop-save-btn">
                    <button type="submit" class="btn btn-primary px-4">Save Batch</button>
                </div>
...
/*
    Auto suggest KHUJLI quantity:
    - KHUJLI = Total BLK Qty × 0.165
    - Field remains editable
    - If user manually edits KHUJLI, system will not overwrite again
*/
...
function renderMobileMaterialCards() {
    ...
    document.querySelectorAll('#raw-material-table tbody tr').forEach(function(row, index) {
        ...
        /*
            Mobile select is only for UI.
            Real form data submits from desktop table rows.
        */
...
document.addEventListener('change', function(e) {
    if (e.target.classList.contains('material-select')) {
        ...
        /*
            ...
            New KHUJLI row can receive auto suggestion again.
        */
...
document.addEventListener('input', function(e) {
    if (e.target.classList.contains('qty-used')) {
        ...
        /*
            If BLK qty changes, refresh KHUJLI suggestion
            but do not overwrite manually edited KHUJLI.
        */
        if (isBlkRow(row)) {
            applyKhujliAutoFormula();
        }
